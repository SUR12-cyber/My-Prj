  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        ...
        const logThreshold = Math.floor(
          diagnosticLogs.length * (nextProgress / 100)
        );

        while (
          currentLogIndex < logThreshold &&
          currentLogIndex < diagnosticLogs.length
        ) {
          const nextLog = diagnosticLogs[currentLogIndex];
      ...
      >
        <div className="flex justify-between items-center border-b border-accentCyan/20 pb-4 mb-6">
          <div className="flex items-center gap-2 text-accentCyan">
            ...
            <span className="font-mono text-sm tracking-wider uppercase font-semibold">
              ADITYA-L1 MISSION CONTROL v1.0.0
            </span>
          </div>

          <span className="font-mono text-xs text-textSec">
            LOC-T: {new Date().toLocaleTimeString()}
          </span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center my-6 py-4">
          ...
          <p className="text-xs text-textMuted mt-1">
            SoLEXS + HEL1OS Telemetry Dashboard
          </p>
        </div>
            ...
            {logs
              ...
              .map((log, idx) => (
                ...
                >
                  ...
                  ) : log.endsWith('complete.') ||
                    log.endsWith('ONLINE (v4.2.1-Alpha)...') ? (
                    <span className="text-successGreen">
                      ✔ {log}
                    </span>
                  ) : (
                    <span className="text-accentCyan/80">
                      ❯ {log}
                    </span>
                  )}
                </motion.div>
          ...
          <AnimatePresence>
            {bootFinished && (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={handleEnter}
                className="bg-linear-to-r from-accentCyan to-accentBlue text-spaceBg font-mono text-sm tracking-wider uppercase font-bold py-2 px-8 rounded-md shadow-neonGlow hover:from-white hover:to-accentCyan transition-all border border-accentCyan/30 cursor-pointer"
              >
